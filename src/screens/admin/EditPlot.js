...
import {updatePlot} from '../../configs/firebase/collections/plots.collections';
import DatePicker from 'react-native-datepicker';
import FontAwesome from 'react-native-vector-icons/FontAwesome';
import Snackbar from 'react-native-snackbar';

const EditPlotsScreen = ({navigation, route}) => {
  const [isLoading, setIsLoading] = useState(false);

  const onFormSubmit = state => {
    setIsLoading(true);
    updatePlot(route.params.data.id, state).then(() => {
      setIsLoading(false);
      Snackbar.show({
        text: 'Plot is Updated',
      ...
      <ScrollView>
        <View style={[common.container, styles.container]}>
          <Formik
            ...
            initialValues={{
              plotSize: route.params.data.plotSize,
              cropType: route.params.data.cropType,
              cropName: route.params.data.cropName,
              plantedDate: route.params.data.plantedDate,
              harvestDate: route.params.data.harvestDate,
              plotAddress: route.params.data.plotAddress,
            }}
              ...
              <>
                ...
                {!isLoading ? (
                  <TouchableHighlight
                    ...
                    <Text style={buttons.buttonText}>Update Plot</Text>
                  </TouchableHighlight>
                ) : (
...
export default EditPlotsScreen;
